...
contract ShibaWarsFactory {
    ...
    bytes32 constant merkleRoot =
        0x1e6bcaf9bed99bb583085bdc0b3386ed666694f09ca69aebf4163eadadd2979c;
    ...
    modifier isSeason() {
        require(
            block.timestamp >= IShibaWars(shibaWars).getSeasonStart() &&
                block.timestamp <=
                IShibaWars(shibaWars).getSeasonStart() + SEASON_DURATION,
    ...
    modifier isUser() {
        require(
            ...
            "SHIBAWARS: CAN NOT BE CALLED BY A SMART CONTRACT!"
        );
        _;
    }
    ...
    function redeemDevReward() public {
        ...
        _shibaInu.transfer(
            devAddress,
            devReward.min(_shibaInu.balanceOf(address(this)))
        );
        devReward = 0;
        ...
        _leash.transfer(
            devAddress,
            devRewardLeash.min(_leash.balanceOf(address(this)))
        );
        devRewardLeash = 0;
        ...
        _flokiInu.transfer(
            devAddress,
            devRewardFloki.min(_flokiInu.balanceOf(address(this)))
        );
        devRewardFloki = 0;
    }
    ...
    function payTheContract(uint256 cost) public {
        ...
        require(
            _shibaInu.balanceOf(msg.sender) >= cost,
            "Shiba Wars: INSUFFICIENT SHIB BALANCE"
        );
        require(
            _shibaInu.allowance(msg.sender, factory) >= cost,
            "Shiba Wars: ALLOW US TO SPEND YOUR SHIB"
        );
    ...
    function payTheContractLeash(uint256 cost) public {
        ...
        // does the buyer has enough shib?
        require(
            _leash.balanceOf(msg.sender) >= cost,
            "Shiba Wars: INSUFFICIENT LEASH BALANCE"
        );
        require(
            _leash.allowance(msg.sender, factory) >= cost,
            "Shiba Wars: ALLOW US TO SPEND YOUR LEASH"
        );
    ...
    function payTheContractFloki(uint256 cost) public {
        ...
        require(
            _floki.balanceOf(msg.sender) >= cost,
            "Shiba Wars: INSUFFICIENT FLOKI BALANCE"
        );
        require(
            _floki.allowance(msg.sender, factory) >= cost,
            "Shiba Wars: ALLOW US TO SPEND YOUR FLOKI"
        );
        // transfer floki from buyer to smart contract
        uint256 balance = _floki.balanceOf(address(this));
        require(
            _floki.transferFrom(msg.sender, factory, cost),
            "Shiba Wars: Can not transfer tokens to the smart contract"
        );
    ...
    function donateShib(uint256 amount) public {
        ...
        // does the buyer has enough shib?
        require(
            _shibaInu.balanceOf(msg.sender) >= amount,
            "Shiba Wars: INSUFFICIENT SHIB BALANCE"
        );
        require(
            _shibaInu.allowance(msg.sender, factory) >= amount,
            "Shiba Wars: ALLOW US TO SPEND YOUR SHIB"
        );
        // transfer shib from buyer to smart contract
        require(
            _shibaInu.transferFrom(msg.sender, factory, amount),
            "Shiba Wars: Can not transfer tokens to the smart contract"
        );
    ...
    function buyShibaWithTrainerTokens(uint256 tokenId) public isSeason isUser {
        ...
        require(
            tokenPrice != 0,
            "SHBIAWARS: This token can not be bought with trianer tokens"
        );
        require(
            trainerTokens[msg.sender] >= tokenPrice,
            "SHIBAWARS: Not enough trainer tokens!"
        );
        trainerTokens[msg.sender] -= tokenPrice;
        uint256 id = shibaWars.mintNFT(msg.sender, tokenId);
        emit TokenBought(id, msg.sender);
    ...
    function recycleShiba(uint256 shibaId) public isSeason {
        ShibaWarsEntity.Shiba memory shiba_ = shibaWars.getTokenDetails(
            shibaId
        );
        require(
            shiba_.inArena == 0,
            "SHIBAWARS: This dog is waiting for a fight!"
        );
    ...
    function openPack(uint256 id) public isUser {
        ...
        // mint random token
        uint256 tokenId = ShibaWarsUtils.getRandomId(
    ...
    function endSeason() public seasonEnded {
        // get top 10
        (uint256[] memory winners, uint256[] memory scores) = shibaWars
            .getWinners();
        for (uint256 i = 0; i < 9; ++i) {
            for (uint256 j = i + 1; j < 10; ++j) {
                if (scores[i] < scores[j]) {
                    uint256 tmp = winners[j];
                    winners[j] = winners[i];
                    winners[i] = tmp;
                    tmp = scores[j];
                    scores[j] = scores[i];
        ...
        uint256 prizepool = winnersReward;
        uint256 prizepoolLeash = winnersRewardLeash;
        uint256 prizepoolFloki = winnersRewardFloki;
        for (uint256 i = 0; i < 10; ++i) {
            address winner = shibaWars.ownerOf(winners[i]);
            uint256 prizeShib = prizepool.ratio(shares[i], 100000);
            uint256 prizeLeash = prizepoolLeash.ratio(shares[i], 100000);
            uint256 prizeFloki = prizepoolFloki.ratio(shares[i], 100000);
    ...
    function claimPrize() public seasonEnded {
        ...
        (uint256 _matchesWon, uint256 _totalMatches) = shibaWarsArena
            .getMatchesWon(msg.sender);
        uint256 shibPrize = matchmakingReward.ratio(
            _matchesWon,
            _totalMatches.mul(10)
        );
        uint256 leashPrize = matchmakingRewardLeash.ratio(
            _matchesWon,
            _totalMatches.mul(10)
        );
        uint256 flokiPrize = matchmakingRewardFloki.ratio(
            _matchesWon,
            _totalMatches.mul(10)
        );
    ...
    function claimablePrize(address user)
        public
        view
        returns (
            uint256 _shib,
        ...
        )
    {
        if (prizeClaimed[user]) {
            ...
            (uint256 _matchesWon, uint256 _totalMatches) = shibaWarsArena
                .getMatchesWon(msg.sender);
            (_shib, _leash, _floki) = _matchesWon == 0 || _totalMatches == 0
                ? (0, 0, 0)
                : (
                    matchmakingReward.ratio(_matchesWon, _totalMatches.mul(10)),
                    matchmakingRewardLeash.ratio(
                        _matchesWon,
                        _totalMatches.mul(10)
                    ),
                    matchmakingRewardFloki.ratio(
                        _matchesWon,
                        _totalMatches.mul(10)
                    )
                );
    ...
    function hasAirdrop(
        ...
        return
            ...
            checkProof(proof, getHash(account, tokenId));
    }
        ...
        isSeason
    {
        require(
            ...
            "Shiba Wars: Airdrop claimed already!"
        );
        require(
            hasAirdrop(proof, msg.sender, tokenId),
            "Shiba Wars: Address is not eligible for this airdrop"
        );
        uint256 id = shibaWars.mintNFT(msg.sender, tokenId);
    ...
    function getHash(address account, uint8 tokenId)
        public
        pure
        returns (bytes32)
    {
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < proof.length; i += 2) {
            el = proof[i];
