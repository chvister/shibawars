...
contract ShibaWars is ERC721 {
    ...
    string constant baseURI =
        "https://ipfs.io/ipfs/QmVMU7GdBnkZg55CedeeVxEehW9v5rbHf391frzoBnYs6q/";
    ...
    modifier isOwner(uint256 tokenId, address caller) {
        require(
    ...
    function mint(
        ...
        _tokenDetails[nextId] = ShibaWarsEntity.Shiba(
            nextId,
            breed[tokenId],
            (uint64)(strength),
            (uint64)(agility),
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    baseURI,
                    (uint256)(_shiba.tokenId).toString(),
                    ".json"
                )
            );
    }
    ...
    function setShibaWarsArena(address shibaWarsArena_)
        public
        isDev(msg.sender)
    {
    ...
    // SETS ADDRESS OF ARENA CONTRACT. CAN ONLY BE CALLED BY DEV
    function setFactoryAddress(address factoryAddress_)
        public
        isDev(msg.sender)
    {
    ...
    function getWinners()
        public
        view
        returns (uint256[] memory result, uint256[] memory scores)
    {
        result = new uint256[](10);
        scores = new uint256[](10);
        uint256 found = 0;
        (uint256 min, uint256 max) = (MAX_INT, 0);
        for (uint256 i = 0; i < nextId; ++i) {
            // if owner and exists
            if (_exists(i)) {
    ...
    function mintNFT(address owner, uint256 tokenId)
        public
        isShibaWars(msg.sender)
        returns (uint256)
    {
        uint256 multiplier = ShibaWarsUtils.getStatsMultiplier(tokenId);

        (uint256 str, uint256 agi, uint256 dex) = (tokenId / 100 == 1 &&
            tokenId > ShibaWarsUtils.SHIBAWARS_SUPPORTER)
            ? (
                abi.encodePacked(block.difficulty, block.timestamp).random(
                    10 * multiplier,
                    16 * multiplier
                ),
                abi.encodePacked(tokenId, block.timestamp).random(
                    10 * multiplier,
                    16 * multiplier
                ),
                abi.encodePacked(block.difficulty, tokenId).random(
                    10 * multiplier,
                    16 * multiplier
                )
            )
        ...
        uint256 power = multiplier == 0
            ? 0
            : str.add(agi).add(dex).percent(48 * multiplier);
    ...
    function levelUp(uint256 id) public isSeason {
        ...
        require(
            shibaTreats[msg.sender] >= ShibaWarsUtils.levelUpCost(_shiba.level),
            "Shiba Wars: NOT ENOUGH POWER TREATS TO UPGRADE THIS SHIBA"
        );
        // only can level up shibas
        require(
            ShibaWarsUtils.isShiba(_shiba.tokenId),
            "Shiba Wars: ONLY SHIBAS CAN BE LEVELLED UP"
        );
        shibaTreats[msg.sender] = shibaTreats[msg.sender].sub(
            ShibaWarsUtils.levelUpCost(_shiba.level)
        );
        ++_tokenDetails[id].level;
        _tokenDetails[id].strength += _shiba.strengthGain;
        _tokenDetails[id].hitPoints = ShibaWarsUtils.getMaxHp(
            _shiba.strength + _shiba.strengthGain
        );
        _tokenDetails[id].agility += _shiba.agilityGain;
        _tokenDetails[id].dexterity += _shiba.dexterityGain;
    }

    function recycle(uint256 id, address caller)
        public
        isShibaWars(msg.sender)
        isOwner(id, caller)
    {
    ...
    function setInArena(uint256 id, uint16 inArena)
        public
        isShibaWars(msg.sender)
    {
    ...
    // DECREASES HP
    function decreaseHp(uint256 id, uint256 damage)
        public
        isShibaWars(msg.sender)
    {
    ...
    // SET SCORE FOR SHIBA
    function setScore(uint256 id, uint128 score)
        public
        isShibaWars(msg.sender)
    {
    ...
    // SET MAX SCORE
    function setMaxScore(uint256 id, uint128 score)
        public
        isShibaWars(msg.sender)
    {
    ...
    function getUserTreatTokens(address user) public view returns (uint256) {
