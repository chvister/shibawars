...
async function createDb() {
    ...
    let blocks = endBlock - startBlock;
    let nextPercent = 0;
    ...
    for(var block = startBlock; block < endBlock; block += step) {
        ...
        await contract.getPastEvents('Transfer', {
            ...
            var percent = ((block - startBlock) / blocks) * 100;
            ...
            events.forEach((item) => {
                if(accountBalances[item["returnValues"]["from"]] === undefined) {
                    accountBalances[item["returnValues"]["from"]] = BigInt(0);
                    theAccounts.push(item["returnValues"]["from"]);
                } else {
                    accountBalances[item["returnValues"]["from"]] -= BigInt(item["returnValues"]["value"]);
                }
                if(accountBalances[item["returnValues"]["to"]] === undefined) {
                    accountBalances[item["returnValues"]["to"]] = BigInt(item["returnValues"]["value"]);
    ...
    console.log("creating file");
        ...
        var csvFile = "";
        var csvFile2 = "";
        let token = "000000000000000000";
        for (var i = 0; i < theAccounts.length; i++) {
            var theAddress = theAccounts[i];
            var amount = accountBalances[theAddress];
            var aidropId = 0;
            if(amount >= BigInt("500000000000000" + token)) {
                aidropId = 2;
            } else if(amount >= BigInt("1000000000000" + token)) {
                aidropId = 3;
            } else if(amount >= BigInt("15000000000" + token)) {
                aidropId = 4;
            } else if(amount >= BigInt("2500000000" + token)) {
                aidropId = 5;
            } else if(amount >= BigInt("10000000" + token)) {
                aidropId = 6;
            }
            if(aidropId != 0){
                csvFile += theAccounts[i] + "0" + aidropId.toString() + `;\n`;
                csvFile2 += theAccounts[i] + ";" + aidropId.toString() + `;\n`;
            }
        }

        var blob = new Blob([csvFile], { type: 'text/csv;charset=utf-8;' });
        var blob2 = new Blob([csvFile2], { type: 'text/csv;charset=utf-8;' });
        if (navigator.msSaveBlob) { // IE 10+
            ...
            if (link.download !== undefined) { // feature detection
                ...
                link.setAttribute("href", url);
                link.setAttribute("download", "airdrop2.csv");
                link.style.visibility = 'hidden';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
